...
## Executive Summary
**Torpedo** is the high-performance compilation and optimization engine for the Modular Quartet. It transforms functional **RecordStream** pipelines and PyTorch models into optimized, production-ready inference artifacts (TorchScript, ONNX, TensorRT).
...
At runtime, RecordStream uses "Boxed" immutable records for safety. Torpedo "unpacks" the box during compilation, creating direct tensor-to-tensor links between nodes. This eliminates the overhead of dictionary lookups and tuple creation in the hot path.
